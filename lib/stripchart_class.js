...
9. this._convTuples: an array of tuples specifying the coefficients for a linear conversion of a row element;
    ...
    if a is negative, this may identify a specific conversion formula (see units.js);
...
class stripChart {
  constructor(SCelem, subscribePaths) {
    ...
    log("s","<br>this._chartSpecs for " + this._SCname + ":<br>", this._chartSpecs);
    ...
    //  Also, prepares the stripChartsClass instance control blocks
      ...
      // add property named 'pathname$source' (with non A-Za-z0-9_ char replaced by "_") and index as value
      if (zPath.$source == "") {
          this._path_Index[(zPath.path).replace(/\W/g,'_')] = this._validPathsNbr;
      }
      else {
          this._path_Index[zPath.path.replace(/\W/g,'_') + "$" + zPath.$source.replace(/\W/g,'_')] = this._validPathsNbr;
      }
