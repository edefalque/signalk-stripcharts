...
class stripChart {
  constructor(SCelem, subscribePaths) {
    ...
    this._chartSpecs = {
            ...
            data: { hide: 'time', rows: this._rows, axes: this._axes, colors: this._colorPalette, type: 'spline' },   
            ...
            point: { show: true, r: options.pointRadius,
  ...
  loadView() {
        ...
        if  ( this._rowCountSinceRefresh >= 1        // at least one new row
              ...
              (    this._rowCountSinceRefresh >= this._intervalsPerRefresh  // normal rate
                ...
                || ws.readyState == ws.CLOSED        // refresh after ws closing
              )
            )
            {
  ...
  getPastData(T1) {
    ...
    var queries = this.prepQueries(T0, T1);

    influxLib.influxQueryRaw(connectedDB, queries, {precision: "ms"})
    .then(           
          rawData => {
            if (!rawData) {
              alert("influx returned no data");
            } else
            {            
  ...
  fillFmInflux(connectedDB, T1) {
    ...
    .then(           
          ...
          error => { alert(error); refreshDisabled = false; }
        );
    return;
  }
  ...
  prepQueries(T0, T1) {
    // prepare queries
    let rpIx = selectRP(this._avgInterval, options.influxDB.retentionPolicies);
    if (rpIx == -1) { alert("no suitable retention pol in influxdb"); return;}
    let q = [];
    this._paths.forEach(pushQuery, this);
    log("i","queries: \n", q);
    return q;

    function selectRP(avgInt, retPolicies) {
      // select rp with largest sampling_ms <= avgInt if any
      // otherwise, select default rp:
      let selectedSamplingIx = 0;

      let selectedSampling = 0;
      for (let i = 0; i < retPolicies.length ; i++) {
        if (selectedSampling < retPolicies[i].sampling_ms && retPolicies[i].sampling_ms <= avgInt  ) {
          selectedSampling = retPolicies[i].sampling_ms;
          selectedSamplingIx = i;
        }
      }
      log("i", "selectedSamplingIx: ", selectedSamplingIx);
      return selectedSamplingIx;
    }

    function pushQuery(path, iPath) {
      let rp = options.influxDB.retentionPolicies[rpIx].RP;
      let rpIsDefault = (rpIx == 0)? true: false;
