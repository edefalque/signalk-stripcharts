...
specs.onload = function() {        // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if (options.useInfluxDB) {
        ...
        setTimeout(function() {
                if (influxRPs.length == 0) {
                    let errMsg =
                    "Timeout(2 sec) loading RPs: Influx not reachable or started, or invalid dbname.\n options.useInfluxDB set to false.";
                    console.log(errMsg);
                    alert(errMsg);
    ...
    // intialize y2Ctlx, playIcon, moonIcon
...
function setInfluxButtons(useIfx) {
    ...
    clockButton.style.display = (useIfx) ? "inline" : "none";
    pausePlayButton.style.border = (useIfx) ? "2px solid #ff0000" : "none";
    return;
}
...
function fillFmInflux(chartObj) {
    ...
    if  (!connectedDB) {
        if (typeof options.influxDB.dbName === "string") {
            connectedDB = influxLib.influxConnect(options.influxDB.dbName, server.slice(0,-5), 8086);
        } else { alert("options.js: missing options.influxDB.dbName") }
    }  
...
calPicker.on('submit', function(aDate, readableDate){
    ...
    if  (!connectedDB) {
        if (typeof options.influxDB.dbName !== "string") {
            alert("options.js: missing options.influxDB.dbName");
            return;
        }
        connectedDB = influxLib.influxConnect(options.influxDB.dbName, server.slice(0,-5), 8086);
    }
